...
class AutoencoderWithRegression(nn.Module):
    ...
    def forward(self, x_data, y_data, idx, sigma, k_nearest, kernel, device):
        ...
        beta = loregs.torch_weighted_regression_with_regularization(encoded_all, y_data.squeeze(0), weights_all_std, device, lambda_reg=0.2)
...
def train_model(input_data,outcome_var, batch_size, switch_shuffle, num_batches, writer, model, optimizer, num_epochs, log_interval, alpha, theta, gamma, device, sigma, k_nearest, kernel):
    """
    ...
    for epoch in tqdm(range(num_epochs), desc="Training"):
        ...
        for batch in data_loader:
            ...
            for mini_batch in mini_batches:
                ...
                # Fit global model
                beta_global = loregs.torch_weighted_regression(encoded_all, Y_target.squeeze(0), weights_global, device)
                ...
                # Regularization of correlation matrix
                ...
                total_loss_reconstruction += loss_reconstruction
                total_loss_null += loss_null
                total_loss_global += loss_global

            # Average losses over mini-batches
            total_loss_reconstruction /= len(mini_batches)
            total_loss_null /= len(mini_batches)
            total_loss_global /= len(mini_batches)

            # Combine losses
            total_loss = alpha * total_loss_reconstruction + theta * total_loss_null + gamma * total_loss_global
        ...
        if (epoch + 1) % log_interval == 0:
            ...
            current_loss_values["loss"].append(total_loss.item())
            current_loss_values["reconstruction_loss"].append(total_loss_reconstruction.item())
            current_loss_values["null_loss"].append(total_loss_null.item())
            current_loss_values["global_loss"].append(total_loss_global.item())

            writer.add_scalar('Loss/Total', total_loss.item(), epoch)
            writer.add_scalar('Loss/Reconstruction', total_loss_reconstruction.item(), epoch)
            writer.add_scalar('Loss/Null', total_loss_null.item(), epoch)
            writer.add_scalar('Loss/Global', total_loss_global.item(), epoch)
